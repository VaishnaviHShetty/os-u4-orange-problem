...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    // Step 1: Build the header string like "blob 42\0"
    ...
    // Step 3: Hash the whole thing
    compute_hash(full, total, id_out);

    // Step 4: If it already exists, skip writing (deduplication)
    if (object_exists(id_out)) { free(full); return 0; }

    // Step 5: Build the path and create the shard directory
    char path[512], tmp_path[520];
    object_path(id_out, path, sizeof(path));
    char dir[512];
    snprintf(dir, sizeof(dir), "%s", path);
    // dir is like ".pes/objects/ab/cdef...", we want ".pes/objects/ab"
    char *slash = strrchr(dir, '/');
    if (slash) *slash = '\0';
    mkdir(dir, 0755); // create .pes/objects/XX/
    ...
    // Step 7: Atomically rename temp -> final
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // Step 2: Read the whole file into memory
    ...
    // Step 3: Verify hash (integrity check)
    ObjectID actual;
    compute_hash(buf, total, &actual);
    ...
    // Step 5: Determine type from header
    ...
    // Step 6: Copy out the data portion (after the \0)
